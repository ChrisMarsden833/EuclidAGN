...
class XLFData(utl.data):
    ...
    def get_ueda14(self, bins, nh='free'):
        ...
        if nh == 'free':
            return utl.PlottingData(10**bins, phi)
...
class EddingtonDistributionData(utl.data):
    """ Class to store the comparison data for the Eddington Ratio distribution.

    :param z: float, redshift.
    """
    def __init__(self, z, datapath="./Data/"):
        utl.data.__init__(self, z, datapath=datapath)

        # Read in Geo17
        geo_lx, geo_phi_top, geo_phi_bottom, z = utl.ReadSimpleFile("Geo17", self.z, self.dataPath, cols=3, retz=True)
        self.Geo = utl.IntervalPlottingData(geo_lx, geo_phi_top, geo_phi_bottom, z=z)

        # Read in Bon 16
        bon16_lx, bon16_phi, z = utl.ReadSimpleFile("Bon16", self.z, self.dataPath, retz=True)
        self.Bon16 = utl.PlottingData(bon16_lx, bon16_phi, z=z)

        # Read in Bon 12
        bon12_lx, bon12_phi, z = utl.ReadSimpleFile("Bon12", self.z, self.dataPath, retz=True)
        self.Bon12 = utl.PlottingData(bon12_lx, bon12_phi, z=z)

        # Aird
        aird_lx, aird_phi, z = utl.ReadSimpleFile("Aird2018", self.z, self.dataPath, retz=True)
        self.Aird = utl.PlottingData(aird_lx, aird_phi, z=z)

        # Aird12
        aird_lx, aird_phi, z = utl.ReadSimpleFile("Aird12", self.z, self.dataPath, retz=True)
        self.Aird12 = utl.PlottingData(aird_lx, aird_phi, z=z)
