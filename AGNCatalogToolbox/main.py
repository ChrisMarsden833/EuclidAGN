...
def halo_mass_to_stellar_mass(halo_mass, z=0, formula="Grylls19", scatter=0.11):
    ...
    This is based on Grylls 2019, but also has the option to use the parameters from Moster. This is a simplified
    version of Pip's DarkMatterToStellarMass() function.
    ...
    if not scatter == False:  # Looks like bad practice, it's actually cool as scatter can also be a float.
...
def to_duty_cycle(method, stellar_mass, black_hole_mass, z=0, data_path="./Data/DutyCycles/"):
    ...
    :param method: string/float. If string, should be a method (currently "Man16", "Schulze" or "Schulze").
...
def bh_mass_to_eddington_ratio_luminosity(black_hole_mass, z=0, method="Schechter", redshift_evolution=False,
                                          parameter1=-1, parameter2=-0.65):
    """ Function to assign the eddington ratios and x_ray luminosity from black hole mass.
...
def weightedFunction(parameter, duty_cycle, bins, volume):
    ...
    lum_bins = sp.stats.binned_statistic(parameter, duty_cycle, "sum", bins=bins)[0]
